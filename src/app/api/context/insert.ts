...
export async function createEmbeddingsTable(url: string, pages: number, isDirect: boolean) {
  ...
  const data = contextualize(await getDomObjects(url, pages, isDirect), 5, 'link')
  const batchSize = 500;
